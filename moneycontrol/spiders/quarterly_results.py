...
class RatiosSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "select concat('http://www.moneycontrol.com/financials/yesbank/results/quarterly-results/',mc_symbol,'#',mc_symbol),mc_symbol,symbol from symbol_mc_map ;"
        )
        ...
        for dataRow in dataRows:
            ...
            if(len(actualData)>len(years)):
                for index in range(0,len(years)):
                    y = str(years[index]);
                    y=y.replace("'",""); # because Sep '17 is the heading
                    y = y.split(" ");
                    finData = RatioItem();
